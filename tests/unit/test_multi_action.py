...
class TestMultiActionInterpreter(unittest.TestCase):
    ...
    def test_react_major_conflict_downgrades_to_inconclusive(self):
        probe_results = responses([
            {
                "action_id": "40aaaa",
                "status_code": 500,
    ...
    def test_all_failed_includes_raw_results_and_journals_failures(self):
        ...
        self.assertEqual(result["raw_data"]["probe_results"], [res.to_mapping() for res in probe_results])
        self.assertEqual(result["raw_data"]["control_results"], [res.to_mapping() for res in control_results])
