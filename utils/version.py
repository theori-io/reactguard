...
def parse_semver(version: Optional[str]) -> Optional[ParsedVersion]:
    if not version:
        return None
...
class VersionPatterns:
    ...
    RSC_FLIGHT_IMPORT = re.compile(
        rf'\d+:I\["react-server-dom-(?:webpack|parcel|turbopack)"\s*,\s*"({SEMVER_PATTERN})"'
    )
    ...
    if versions.get("react_version"):
        ...
        if react_ver and react_ver[0].isdigit():
            try:
                ...
                versions["react_major_confidence"] = versions.get(
                    "react_version_confidence", "medium"
                )
            except ValueError:
