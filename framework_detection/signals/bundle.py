...
def probe_js_bundles(
    ...
    *,
    ...
    for script_src in all_js_urls:
        ...
        has_v7_manifest = (
            "__reactRouterManifest" in js_content or "__reactRouterContext" in js_content
        )
        has_v7_pkg_literal = re.search(r"react-router(?:-dom)?@7", js_content) is not None
        has_v7_version = (
            re.search(r'"7\.\d+\.\d+"', js_content) is not None
            and "react-router" in js_content.lower()
        )
        has_v7_router = (
            has_v7_manifest
            or has_v7_pkg_literal
            or (has_v7_version and "createBrowserRouter" in js_content)
        )
        if has_v7_router:
            signals["react_router_v7_bundle"] = True

        has_react_router_pkg = (
            "react-router-dom" in js_content or "react-router" in js_content.lower()
        )
        has_v6_routes = (
            has_react_router_pkg
            and "Routes" in js_content
            and "Route" in js_content
            and "Switch" not in js_content
        )
        has_v6_data_router = has_react_router_pkg and (
            "createBrowserRouter" in js_content or "RouterProvider" in js_content
        )
        ...
        if not signals.get("react_router_v7_bundle"):
            if has_v6_data_router:
                ...
                has_react_router_pkg
                and re.search(r'"6\.\d+\.\d+"', js_content)
                and "Routes" in js_content
            ):
                signals["react_router_v6_bundle"] = True
            elif has_v6_routes:
        ...
        if not signals.get("react_router_v6_bundle"):
            if (
                has_react_router_pkg
                and ("BrowserRouter" in js_content or "browserRouter" in js_content)
                and "Switch" in js_content
            ):
                signals["react_router_v5_bundle"] = True

        has_expo_router = "expo-router" in js_content and (
            "renderRouter" in js_content or "EXPO_ROUTER" in js_content
        )
        if has_expo_router:
