...
WAKU_META_GENERATOR_PATTERN = re.compile(
    r'<meta[^>]+name=["\']generator["\'][^>]+content=["\']Waku["\']', re.IGNORECASE
)
...
GENERIC_FLIGHT_PAYLOAD_PATTERN = re.compile(
    r'^\d+:(?:\["\$|\{\s*"a"\s*:\s*"\$|\{\s*"id"\s*:\s*"\$)', re.MULTILINE
)
...
SPA_MODULEPRELOAD_PATTERN = re.compile(
    r'rel=["\']modulepreload["\']\s+href=["\']/assets/[^\s"\']+\.js'
)

# RSC probe markers.
RSC_PROBE_FLIGHT_BODY_PATTERN = re.compile(
    r'^0:(?:\["\$|' r'\[null,\["\$|' r'\{"a"\s*:\s*"\$|' r'I\["react-server-dom-)',
    re.IGNORECASE,
)
...
SERVER_ACTIONS_ACTION_KEYWORDS = (
    SERVER_ACTIONS_STRONG_ACTION_KEYWORDS + SERVER_ACTIONS_GENERIC_ACTION_KEYWORDS
)
