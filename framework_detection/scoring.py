...
STRONG_SIGNAL_WEIGHTS = (
    _config.get("strong_signal_weights", _DEFAULT_STRONG_SIGNAL_WEIGHTS)
    if _config
    else _DEFAULT_STRONG_SIGNAL_WEIGHTS
)
SUPPORTING_SIGNAL_WEIGHTS = (
    _config.get("supporting_signal_weights", _DEFAULT_SUPPORTING_SIGNAL_WEIGHTS)
    if _config
    else _DEFAULT_SUPPORTING_SIGNAL_WEIGHTS
)
...
def score_confidence(signals: Dict[str, Any]) -> Tuple[int, str, Dict[str, Any]]:
    ...
    router_confidence = signals.get("react_router_confidence")
    ...
    has_only_mutable = (
        not has_strong
        and supporting_hits
        and all(sig in {"header_powered_by_nextjs", "vite_assets"} for sig in supporting_hits)
    )
